...
public class UserManager {
    ...
    public AUser findById(Long userId) {
        try {
            if (logger != null) logger.info("Deleting user with ID: {}", userId);
            return m_user_repository.findById(userId);
        } catch (Exception e) {
            if (logger != null) logger.error("Failed to delete user with ID {}: {}", userId, e.getMessage(), e);
            return null;
        }
    }
}
