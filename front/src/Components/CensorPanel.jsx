...
export default function CensorPanel() {
    ...
    const censorQuest = (id) => {
        Backend.censorQuestionnaire(id)
            .then(() =>
                setQuests(prev => prev.map(q => q.id === id ? { ...q, censored: true } : q))
            )
            .catch(err => console.error(err));
    };
